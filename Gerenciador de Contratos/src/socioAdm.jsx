...
export async function estruturaSocioAdm(idendereco, nome, cpf, orgaoemissor, estadocivil, nacionalidade, idsocio = "", cnpj){
    try{
      if (idsocio == ""){
        const socio = await invoke("estrutura_socio_adm", {idendereco, nome, cpf, orgaoemissor, estadocivil, nacionalidade, cnpj});
        return socio;
      }
      const socio = await invoke("estrutura_primeiro_socio", {idendereco, nome, cpf, orgaoemissor, estadocivil, nacionalidade, idsocio, cnpj});
      return socio;
    } catch(error) {
...
export async function cadastraSocioAdm(idendereco, nome, cpf, orgaoemissor, estadocivil, nacionalidade, idsocio = "", cnpj = ""){
    try{
      const socioadm = await estruturaSocioAdm(idendereco, nome, cpf, orgaoemissor, estadocivil, nacionalidade, idsocio, cnpj);
...
export const selecionaNacionalidadeDefinido = (setNacionalidade, selectedNacionalidade = "", isReadOnly = false, widthvh = 65, marginleft = 9) => {
  return (
    <div className= "input-box">
            <select id= "nacionalidade" name= "nacionalidade" required aria-label= "Nacionalidade do sócio Administrativo"
              style={{width: widthstr(widthvh), marginLeft: marginstr(marginleft)}}
...
export const selecionaEstadoCivilDefinido = (setEstadoCivil,  selectedEstadoCivil = "", isReadOnly = false, widthvh = 65, marginleft = 9) => {
  return (
    <div class= "input-box">
            <select id= "estadoCivil" name= "estadoCivil" required aria-label= "Estado civil do socio Administrativo"
              style={{width: widthstr(widthvh), marginLeft: marginstr(marginleft)}}
