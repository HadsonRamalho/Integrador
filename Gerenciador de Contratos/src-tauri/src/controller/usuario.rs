...
pub async fn atualiza_email(input: Json<AtualizaEmailInput>) -> Result<StatusCode, (StatusCode, Json<String>)>{
    ...
    if !resultado_busca2{
        return Err((StatusCode::BAD_REQUEST, Json("O e-mail novo já pertence a outro usuário.".to_string())))
    }   
...
pub async fn atualiza_senha(input: Json<VerificaSenhaInput>) -> Result<StatusCode, (StatusCode, Json<String>)>{
    ...
    match resultado_atualizacao{
        ...
        Err(_e) => {
            ...
            return Err((StatusCode::BAD_REQUEST, Json(MeuErro::AtualizarSenhaUsuario.to_string())))
        }
    }
}
...
pub async fn verifica_token(input: Json<VerificaTokenInput>) -> Result<StatusCode, (StatusCode, Json<String>)>{
    let email = input.email.clone();
    ...
    match id{
        Ok(id) =>{
            if id.is_empty(){
                return Err((StatusCode::BAD_REQUEST, Json("Erro ao validar o token: Verifique o email.".to_string())))
            }
            uid = id;
        },
        Err(e) =>{
    ...
    match email{
        Ok(_) =>{
            ...
            return Err((StatusCode::BAD_REQUEST, Json("Token inválido.".to_string())));
        },
        Err(e) => {
            return Err((StatusCode::INTERNAL_SERVER_ERROR, Json(e.to_string())))            
        }
...
//#[tauri::command]
pub async fn busca_id(input: Json<EmailInput>) -> Result<(StatusCode, Json<String>), (StatusCode, Json<String>)>{
    ...
    match resultado_busca{
        Ok(id) =>{
            if id.is_empty(){
                return Err((StatusCode::BAD_REQUEST, Json("ID não encontrado. Verifique o e-mail.".to_string())));
            }
...
//#[tauri::command]
pub async fn busca_nome_usuario(input: Json<String>) -> Result<(StatusCode, Json<String>), (StatusCode, Json<String>)>{
    let id = input.0;
    if id.trim().is_empty(){
        return Err((StatusCode::BAD_REQUEST, Json("Erro: O ID está vazio".to_string())))
    }
...
//#[tauri::command]
pub async fn busca_cnpj_usuario(input: Json<String>) -> Result<(StatusCode, Json<String>), (StatusCode, Json<String>)>{
    let id = input.0;
    if id.trim().is_empty(){
    ...
    match resultado_busca{
        ...
        Err(_) => {
            return Err((StatusCode::BAD_REQUEST, Json(MeuErro::CnpjNaoEncontrado.to_string())));
        }
    }
}
...
//#[tauri::command]
pub async fn atualiza_nome(input: Json<AtualizaNomeInput>) -> Result<StatusCode, (StatusCode, Json<String>)>{
    let email = input.email.clone();
    let nome = input.nome.clone();
    let email = email.trim();
    if email.is_empty() {
        return Err((StatusCode::BAD_REQUEST, Json(MeuErro::EmailVazio.to_string())))
    }
    if nome.is_empty(){
        return Err((StatusCode::BAD_REQUEST, Json(MeuErro::NomeVazio.to_string())))
    }
    let resultado_busca = busca_id_usuario( email).await.is_ok();
    if !resultado_busca{
        return Err((StatusCode::BAD_REQUEST, Json(MeuErro::EmailNaoEncontrado.to_string())))      
    }
